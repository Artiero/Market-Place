    <!-- Bootstrap core JavaScript-->
    <script src="../assets-cms/vendor/jquery/jquery.min.js"></script>
    <script src="../assets-cms/vendor/bootstrap/js/bootstrap.bundle.min.js"></script>

    <!-- Core plugin JavaScript-->
    <script src="../assets-cms/vendor/jquery-easing/jquery.easing.min.js"></script>

    <!-- Custom scripts for all pages-->
    <script src="../assets-cms/js/sb-admin-2.min.js"></script>

    <!-- Page level plugins -->
    <script src="../assets-cms/vendor/datatables/jquery.dataTables.min.js"></script>
    <script src="../assets-cms/vendor/datatables/dataTables.bootstrap4.min.js"></script>

    <!-- Page level custom scripts -->
    <script src="../assets-cms/js/demo/datatables-demo.js"></script>
    <script src="../assets-cms/js/bootstrap.bundle.min.js"></script>
    <script src="../assets-cms/js/sweetalert.min.js"></script>